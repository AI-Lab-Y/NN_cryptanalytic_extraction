...
def recover_gamma_p(x, ws, bs, ms=10**(-3), precision=10**(-8)):
    ...
    for j in range(1, d_0):
        ...
        suitable_ms_p = utils.binary_search(st=x_index, di=di_j, ws=ws, bs=bs,
                                            ms_low=ms / 10000, ms_high=ms * 10000,
                                            precision=precision)
        suitable_ms_n = utils.binary_search(st=x_index, di=di_j, ws=ws, bs=bs,
                                            ms_low=-1 * ms * 10000, ms_high=-1 * ms / 10000,
                                            precision=precision)
...
if __name__ == '__main__':
    ...
    # nn_shape = [784, 2, 1]
    # nn_path = './models/mnist/mnist_8vs9_784_2_1.npz'

    # nn_shape = [3072, 2, 1]
    # nn_path = './models/cifar10/cifar10_8vs9_3072_2_1.npz'

    nn_shape = [512, 2, 1]
    nn_path = './models/simulated/1_deep_nn_512_2_1.npz'
